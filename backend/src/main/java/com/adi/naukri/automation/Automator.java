 ...
 * REST-layer tests can inject a fake implementation without launching a real browser.</p>
 *
...
public interface Automator {
     ...
     * @param gate     callback that blocks until manual login is complete (only invoked
     *                 when {@code cfg.manualLogin()} is {@code true}).
     * @param listener receives step-level events as they happen.
     * @return ordered list of {@link StepResult}; list is truncated at the first failure.
     */
    List<StepResult> run(
            String          email,
            String          password,
     ...
     *
     * <p>Must be idempotent — safe to call when no run is in progress, and safe to call
     * multiple times. Implementations that do not manage a real browser may leave this
     * as the default no-op.</p>
     */
    default void abort() {
